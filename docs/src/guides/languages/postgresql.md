...
where your Earthly file lies in as a source dir for formatting and run `+FORMAT` Function target.
Under the hood `+FORMAT` Function target runs `sqlfluff-image` docker image,
...
and fully defined under the `+BUILD` Function target.
