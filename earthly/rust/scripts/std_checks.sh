...
status 0 "Checking Rust Code Format" cargo +nightly fmtchk
rc=$?
if [[ "${rc}" -ne 0 ]]; then
...
check_vendored_files "${rc}" .cargo/config.toml "${CARGO_HOME:?}"/config.toml
rc=$?
check_vendored_files "${rc}" rustfmt.toml /stdcfgs/rustfmt.toml
rc=$?
check_vendored_files "${rc}" .config/nextest.toml /stdcfgs/nextest.toml
rc=$?
check_vendored_files "${rc}" clippy.toml /stdcfgs/clippy.toml
rc=$?
check_vendored_files "${rc}" deny.toml /stdcfgs/deny.toml
rc=$?

# Check if we have unused dependencies declared in our Cargo.toml files.
status "${rc}" "Checking for Unused Dependencies" cargo machete
rc=$?

# Check if we have any supply chain issues with dependencies.
status "${rc}" "Checking for Supply Chain Issues" cargo deny check
rc=$?
