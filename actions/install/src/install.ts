...
export async function run(
  ...
  return new Promise((_, reject) => {
    exec(
      'go install -v github.com/input-output-hk/catalyst-ci/cli/cmd@468cdc9e4763b49f639c11186115cd0d782c8dbf && ls /usr/local/bin/ && echo GOBIN && echo $GOBIN && ls -la $GOBIN && echo && GOBININ && ls -la $GOBIN/cli',
      (err, stdout, stderr) => {
        if (err || stderr) {
