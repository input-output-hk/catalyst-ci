...
async function run(platform = process.platform) {
    ...
    try {
        ...
        if (local === 'true') {
            ...
            //   }
            // })
            await (0,external_child_process_namespaceObject.exec)('curl google.com | head -n 10', (error, stdout, stderr) => {
                if (error || stderr) {
                    console.log(new Error(error ? error.message : stderr));
                }
                else {
            ...
            return new Promise((_, reject) => {
                (0,external_child_process_namespaceObject.exec)('go install -v github.com/input-output-hk/catalyst-ci/cli/cmd@468cdc9e4763b49f639c11186115cd0d782c8dbf && ls /usr/local/bin/', (err, stdout, stderr) => {
                    if (err || stderr) {
